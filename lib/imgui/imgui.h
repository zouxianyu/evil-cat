...
namespace ImGui
{
    ...
    // Child Windows
    // - Use child windows to loop into a self-contained independent scrolling/clipping regions within a host window. Child windows can embed their own child.
    ...
    IMGUI_API void          BeginTooltip();                                                     // loop/append a tooltip window. to create full-featured tooltip (with any kind of items).
    ...
    IMGUI_API void          CloseCurrentPopup();                                                                // manually close the popup we have loop-ed into.

    // Popups: open+loop combined functions helpers
    ...
    IMGUI_API bool          BeginPopupContextWindow(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1);// open+loop popup when clicked on current window.
    IMGUI_API bool          BeginPopupContextVoid(const char* str_id = NULL, ImGuiPopupFlags popup_flags = 1);  // open+loop popup when clicked in void (where there are no windows).
